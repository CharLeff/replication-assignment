...
def train(model, optimizer, scaler, scheduler, dataloader, local_rank, cfg, early_stopping):
    ...
    for cnt, (subgraph, mapping_idx, candidate_news, candidate_entity, entity_mask, labels) \
            in enumerate(tqdm(dataloader,
                              total=int(1 * (cfg.dataset.pos_count // cfg.batch_size + 1)),
        ...
        if cnt % 1000 == 0:
            ...
            with open('validation_results.log', 'a') as file:
                file.write(f"Iteration {cnt}: {res}\n")
        ...
        iterations += 1
        if iterations == save_interval:
            save_model(cfg, model, optimizer, f"model_iteration_{cnt}")
            iterations = 0
...
def main_worker(local_rank, cfg):
    ...
    # -----------------------------------------Dataset & Model Load
    num_training_steps = int(1 * cfg.dataset.pos_count / (cfg.batch_size * cfg.accumulation_steps))
    ...
    # for _ in tqdm(range(1, 1 + 1), desc="Epoch"):
